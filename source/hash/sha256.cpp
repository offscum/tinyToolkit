...
namespace tinyToolkit
{
	namespace hash
	{
		const uint32_t SHA256::BLOCK_SIZE;
		const uint32_t SHA256::DIGEST_SIZE;
		...
		void SHA256::Transform(Context & context, const uint8_t * block, uint32_t number)
		{
			...
			#define SHA256_F1(x) (ROT_R(x, 2) ^ ROT_R(x, 13) ^ ROT_R(x, 22))
			#define SHA256_F2(x) (ROT_R(x, 6) ^ ROT_R(x, 11) ^ ROT_R(x, 25))
			#define SHA256_F3(x) (ROT_R(x, 7) ^ ROT_R(x, 18) ^ SHF_R(x, 3))
			#define SHA256_F4(x) (ROT_R(x, 17) ^ ROT_R(x, 19) ^ SHF_R(x, 10))
